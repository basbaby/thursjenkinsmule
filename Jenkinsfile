   ...
   stages {
        	...
        	steps {
					...
					container_Up = false
                    			timeout(time: 1, unit: "HOURS") { 
                        			sh "curl -u admin:admin -X GET -H \"Accept: application/json\" http://104.248.169.167:9000/api/qualitygates/project_status?projectKey=com.mycompany:thursjenkinsmule > status.json"
	 ...
	 stage('Build') {
        ...
        stage('Deploy to Cloudhub'){
        	steps {
				 withCredentials([file(credentialsId: 'settings', variable: 'settings')]){
        	    	sh "mvn -f pom.xml -s $settings package deploy -DskipTests -Dusername=joji1 -Dpassword=Canadavisa25@ -DapplicationName=thursjenkinsmule -Denvironment=Sandbox -DmuleDeploy -Dcloudhub.region=us-east-2 -Danypoint.platform.client_id=fda777bd3e3b4fcb93aff995fea2043d -Danypoint.platform.client_secret=4193AA1986054C548Bf757fd1B7F6f18 -Dkey=mymulesoft"
		...
		stage('Integrate Grafana'){
			steps{
				sh "curl --location --request POST 'http://104.248.169.167:8087/logs'  --header 'Content-Type: application/json' --data-raw '{\"username\": \"joji1\",\"password\": \"Canadavisa25@\",\"orgName\": \"NJC POC\",\"envName\": \"Sandbox\", \"appNames\": [\"thursjenkinsmule\"]}'"
			}
		}	
